...
def run_scan():
    ...
    try:
        ...
        test.table("anomalies").select("id").limit(1).execute()
        print("✅ Supabase connection OK")
    except Exception as e:
        print(f"❌ Supabase connection failed: {e}")
        print("   Check SUPABASE_URL and SUPABASE_SERVICE_KEY secrets in GitHub")
        raise
    ...
    for i, country in enumerate(COUNTRIES):
        ...
        if pn:
            try:
                ...
                # Try realtime trending first, fall back to regular trending
                try:
                    trending_df = pytrends.realtime_trending_searches(pn=pn)
                    trending_terms = trending_df['title'].head(20).tolist() if 'title' in trending_df.columns else []
                    if not trending_terms:
                        raise Exception("No title column")
                except Exception:
                    trending_df = pytrends.trending_searches(pn=pn)
                    trending_terms = trending_df[0].head(20).tolist() if not trending_df.empty else []
